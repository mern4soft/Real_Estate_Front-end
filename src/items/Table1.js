...
export default function SortableTable({TABLE_ROWS,TABLE_HEAD,TABLE_BUTTON, TABLE_HEADING, }) {
...
const handleSubmit = async () => {
  try {
    ...
    const response = await axios.post('https://real-estate-backend-1-6bdi.onrender.com/api/createtypeofprop', propertyData);
...
const handleOpen = () => setOpen(!open);
  ...
  const keys = Object.keys(displayedRows[0] || {});
        ...
        <CardBody className="overflow-scroll px-0">
          <table className="mt-4 w-full min-w-max table-auto text-left">
            <thead>
              <tr>
                {keys?.map((key) => (
                  <th key={key} className="cursor-pointer border-y border-blue-gray-100 bg-blue-gray-50/50 p-4 transition-colors hover:bg-blue-gray-50">
