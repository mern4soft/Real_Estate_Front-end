...
function AgentProperty() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let getProperty = await axios.get('http://localhost:3001/createtypeofprop')
        setData(getProperty.data)
        let getPropertyofAgent = await axios.get(`http://localhost:3001/property/${id}`)
  ...
  const handleSubmit= async() =>{
    try {
      ...
      if (!propertyData._id) {
        const submit = await axios.post('http://localhost:3001/createtypeofprop', propertyData);
        console.log(submit);
      } else {
        
        const update = await axios.put(`http://localhost:3001/createtypeofprop/${propertyData._id}`, propertyData);
        console.log(update);
      }
  ...
  const handleDelete = async(item) =>{
    try {
      ...
      let response =  await axios.delete(`http://localhost:3001/createtypeofprop/${item._id}`)
